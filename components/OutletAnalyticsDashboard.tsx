    ...
    <div className="space-y-6 px-4 pb-12 pt-2">
      <section>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          {analytics.canViewAdminAnalytics && <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3"><CircleDollarSign size={15} className="text-konjo-red" /><p className="mt-2 break-words font-display text-sm font-bold tabular-nums text-konjo-cream sm:text-lg">{analytics.loading ? '…' : formatEtb(unpaidValue)}</p><p className="text-[9.5px] uppercase tracking-wide text-konjo-cream/35">Pending payment</p></div>}
      ...
      <section>
        ...
        <div className="mt-2 grid grid-cols-1 gap-2 min-[390px]:grid-cols-3"><div className="rounded-xl border border-white/10 bg-white/[0.035] p-3"><p className="break-words text-xs font-bold text-konjo-cream">{formatEtb(currentValue.subtotal)}</p><p className="text-[8.5px] uppercase text-konjo-cream/30">Before tax</p></div><div className="rounded-xl border border-white/10 bg-white/[0.035] p-3"><p className="break-words text-xs font-bold text-konjo-amber">{formatEtb(currentValue.tax)}</p><p className="text-[8.5px] uppercase text-konjo-cream/30">Tax</p></div><div className="rounded-xl border border-white/10 bg-white/[0.035] p-3"><p className="break-words text-xs font-bold text-konjo-green">{formatEtb(currentValue.total)}</p><p className="text-[8.5px] uppercase text-konjo-cream/30">Including tax</p></div></div>
        <div className="mt-2 space-y-2">{stockValuation.map(({ product, bottles, subtotal, tax, total }) => <article key={product.id} className="rounded-xl border border-white/10 bg-white/[0.035] p-3"><div className="flex flex-col gap-2 min-[390px]:flex-row min-[390px]:items-start min-[390px]:justify-between"><div className="min-w-0"><p className="break-words text-xs font-semibold text-konjo-cream">{product.name}</p><p className="break-words text-[9.5px] leading-relaxed text-konjo-cream/35">{bottles} bottles · {productPackSize(product)} bottles/pack · {formatEtb(Number(product.unit_price_etb ?? 0))}/bottle</p></div><p className="break-words text-xs font-bold text-konjo-green">{formatEtb(total)}</p></div><p className="mt-1 break-words text-[9px] leading-relaxed text-konjo-cream/30">{formatEtb(subtotal)} before tax + {formatEtb(tax)} tax ({(Number(product.tax_rate ?? 0) * 100).toFixed(2)}%)</p></article>)}</div>
      </section>
      ...
      <section>
        ...
        <div className="mt-2 space-y-2">{analytics.deliveries.map((delivery) => <article key={delivery.id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-3"><div className="flex flex-col gap-2 min-[390px]:flex-row min-[390px]:items-start min-[390px]:justify-between"><div className="min-w-0"><p className="break-words text-xs font-semibold text-konjo-cream">{delivery.product_name}</p><p className="break-words text-[9.5px] leading-relaxed text-konjo-cream/35">{delivery.quantity_entered} {delivery.quantity_unit.toLowerCase()}{delivery.quantity_entered === 1 ? '' : 's'} × {delivery.bottles_per_pack} bottles/pack = {delivery.quantity_bottles} bottles</p></div><p className="break-words text-xs font-bold text-konjo-green">{formatEtb(delivery.total_amount_etb)}</p></div><p className="mt-1 break-words text-[9.5px] leading-relaxed text-konjo-cream/40">{formatEtb(delivery.unit_price_etb)}/bottle · {formatEtb(delivery.subtotal_etb)} before tax + {formatEtb(delivery.tax_amount_etb)} tax ({(delivery.tax_rate * 100).toFixed(2)}%)</p><p className="mt-1 text-[9px] text-konjo-cream/25">Saved by {delivery.username} · {formatEAT(delivery.timestamp)}</p></article>)}{!analytics.loading && !analytics.deliveries.length && <p className="rounded-xl border border-dashed border-white/10 p-5 text-center text-xs text-konjo-cream/30">No priced deliveries have been recorded yet. Older unpriced stock logs remain visible in the operations tracker.</p>}</div>
      </section>
