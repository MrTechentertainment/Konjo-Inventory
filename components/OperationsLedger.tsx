...
import { sourceLocation, sourceLocationLabel, type SourceLocation } from '@/lib/sourceProvenance';
...
export default function OperationsLedger() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [productResult, creditResult, orderResult, sampleResult, inventoryResult] = await withTimeout(async (signal) => await Promise.all([
        ...
        supabase.from('credit_sales').select('id,legacy_reference,payment_status,total_price,refilled_date_raw,due_date_raw,payment_date_raw,bottle_price_raw,agreement_period,payment_type,notes,sales_representative,record_status,quality_notes,source_name,source_sheet,source_row,raw_row,outlets(name,address,subcity),credit_sale_items(quantity,products(sku,name))').order('refilled_date', { ascending: false, nullsFirst: false }).limit(1000).abortSignal(signal),
        supabase.from('sales_orders').select('id,legacy_reference,status,order_date_raw,delivery_date_raw,notes,record_status,quality_notes,source_name,source_sheet,source_row,raw_row,outlets(name,address,subcity),sales_order_items(quantity,products(sku,name))').order('order_date', { ascending: false, nullsFirst: false }).limit(1000).abortSignal(signal),
        supabase.from('product_samples').select('id,legacy_reference,phone,supermarket_representative,sales_representative,date_given_raw,remark,status,feedback,record_status,quality_notes,source_name,source_sheet,source_row,raw_row,outlets(name,address,subcity)').order('date_given', { ascending: false, nullsFirst: false }).limit(1000).abortSignal(signal),
        supabase.from('inventory_ledger_entries').select('id,entry_date_raw,reason,notes,sales_representative,source_name,source_sheet,source_row,raw_row,record_status,quality_notes,inventory_ledger_items(produced_quantity,sample_out_quantity,supermarket_out_quantity,ending_stock,products(sku,name))').order('entry_date', { ascending: false, nullsFirst: false }).limit(1000).abortSignal(signal),
      ...
      const credits: RecordRow[] = (creditResult.data ?? []).map((raw: any) => {
        ...
        return { id: raw.id, kind: 'CREDIT', recordStatus: raw.record_status ?? 'COMPLETE', qualityNotes: safeJsonArray(raw.quality_notes), legacyReference: raw.legacy_reference, outlet, status: raw.payment_status, primaryDate: raw.refilled_date_raw, secondaryDate: raw.due_date_raw, thirdDate: raw.payment_date_raw, totalPrice: raw.total_price === null ? null : Number(raw.total_price), items: (raw.credit_sale_items ?? []).map((item: any) => { const product = relation(item.products); return { sku: product?.sku ?? '', name: product?.name ?? '', quantity: Number(item.quantity) }; }).filter((item: Item) => item.sku), fields: { bottle_price_raw: raw.bottle_price_raw, agreement_period: raw.agreement_period, payment_type: raw.payment_type, notes: raw.notes, sales_representative: raw.sales_representative }, source: sourceLocation({ sourceName: raw.source_name, sourceSheet: raw.source_sheet, sourceRow: raw.source_row, rawRow: raw.raw_row }) };
      });
      const orders: RecordRow[] = (orderResult.data ?? []).map((raw: any) => {
        const outlet = relation(raw.outlets) ?? { name: 'Missing outlet', address: null, subcity: null };
        return { id: raw.id, kind: 'ORDER', recordStatus: raw.record_status ?? 'COMPLETE', qualityNotes: safeJsonArray(raw.quality_notes), legacyReference: raw.legacy_reference, outlet, status: raw.status, primaryDate: raw.order_date_raw, secondaryDate: raw.delivery_date_raw, thirdDate: null, totalPrice: null, items: (raw.sales_order_items ?? []).map((item: any) => { const product = relation(item.products); return { sku: product?.sku ?? '', name: product?.name ?? '', quantity: Number(item.quantity) }; }).filter((item: Item) => item.sku), fields: { notes: raw.notes }, source: sourceLocation({ sourceName: raw.source_name, sourceSheet: raw.source_sheet, sourceRow: raw.source_row, rawRow: raw.raw_row }) };
      });
      const samples: RecordRow[] = (sampleResult.data ?? []).map((raw: any) => {
        const outlet = relation(raw.outlets) ?? { name: 'Missing outlet', address: null, subcity: null };
        return { id: raw.id, kind: 'SAMPLE', recordStatus: raw.record_status ?? 'COMPLETE', qualityNotes: safeJsonArray(raw.quality_notes), legacyReference: raw.legacy_reference, outlet, status: raw.status, primaryDate: raw.date_given_raw, secondaryDate: null, thirdDate: null, totalPrice: null, items: [], fields: { phone: raw.phone, supermarket_representative: raw.supermarket_representative, sales_representative: raw.sales_representative, remark: raw.remark, feedback: raw.feedback }, source: sourceLocation({ sourceName: raw.source_name, sourceSheet: raw.source_sheet, sourceRow: raw.source_row, rawRow: raw.raw_row }) };
      });
  ...
  const filtered = useMemo(() => rows.filter((row) => row.kind === tab && `${row.outlet.name} ${row.outlet.address ?? ''} ${row.legacyReference ?? ''} ${row.status} ${sourceLocationLabel(row.source)}`.toLowerCase().includes(query.trim().toLowerCase())), [query, rows, tab]);
